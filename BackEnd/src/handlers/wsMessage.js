...
const isShipSunk = (shipId, occupiedCells, hitsList) => {
  ...
  return shipCells.every((sc) =>
    hitsList.some((h) => h.row === sc.row && h.col === sc.col),
  );
};
...
const getRealUserIdAndName = async (email, fallbackId, fallbackName) => {
  const cleanFallbackId = fallbackId
    ? String(fallbackId).split(":")[0]
    : fallbackId;
  if (!email) return { userId: cleanFallbackId, username: fallbackName };
  try {
    const resEmail = await documentClient.send(
      new GetCommand({
    ...
    const resUser = await documentClient.send(
      new GetCommand({
        TableName: "User",
    ...
    if (!resUser.Item)
      return { userId: resEmail.Item.userId, username: fallbackName };

    return {
      userId: resUser.Item.userId,
      username: resUser.Item.username || fallbackName,
    };
  } catch (e) {
...
const updateRankedStats = async ({
  ...
  const winnerRank =
    winnerUser.rank || getRankForRp(winnerUser.rankPoints || 0).id;
  const loserRank =
    loserUser.rank || getRankForRp(loserUser.rankPoints || 0).id;
...
const saveMatch = async ({ room, winnerId, leaverId = null }) => {
  ...
  const p1 = await getRealUserIdAndName(
    player1.email,
    player1.userId,
    player1.displayName || "Guest",
  );
  const p2 = await getRealUserIdAndName(
    player2.email,
    player2.userId,
    player2.displayName || "Guest",
  );
  ...
  const realWinnerId = await getRealUserIdAndName(
    winnerPlayer.email,
    winnerPlayer.userId,
    "Guest",
  ).then((res) => res.userId);
  ...
  if (mode === "ranked") {
    const winnerStats = {
      ...
      hits:
        realWinnerId === p1.userId
          ? player1Shots - player1Misses
          : player2Shots - player2Misses,
  ...
  if (!realWinnerId.startsWith("guest_")) {
    await documentClient.send(
      new UpdateCommand({
        ...
        UpdateExpression:
          "SET wins = if_not_exists(wins, :zero) + :one, totalGames = if_not_exists(totalGames, :zero) + :one",
  ...
  if (!loserId.startsWith("guest_")) {
    await documentClient.send(
      new UpdateCommand({
        ...
        Key: { userId: loserId },
        UpdateExpression:
          "SET losses = if_not_exists(losses, :zero) + :one, totalGames = if_not_exists(totalGames, :zero) + :one",
        ExpressionAttributeValues: { ":zero": 0, ":one": 1 },
      }),
    );
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === "SHOOT") {
      ...
      const connRes = await documentClient.send(
        new GetCommand({
      ...
      // 2. Get Room
      const roomRes = await documentClient.send(
        new GetCommand({
      ...
      // Sanitize turn owner
      const cleanTurnUserId = room.currentTurnUserId
        ? String(room.currentTurnUserId).split(":")[0]
        : "";

      // 3. Verify turn
      if (cleanTurnUserId !== cleanUserId) {
        console.log(
          `[SHOOT] Turn verification failed: room.currentTurnUserId=${room.currentTurnUserId} (clean=${cleanTurnUserId}), connection userId=${rawUserId} (clean=${cleanUserId})`,
        );
      ...
      const alreadyShot =
        currentHits.some((h) => h.row === row && h.col === col) ||
      ...
      console.log(
        "2. Tọa độ tàu địch (opponentCoordinates):",
        opponentCoordinates,
      );
      console.log(
        "3. Tàu địch gốc (chưa flatten):",
        JSON.stringify(opponentShips),
      );
      // ===================================
      ...
      const hitCell = isHit
        ? occupiedCells.find((c) => c.row === row && c.col === col)
        : null;
      ...
      const finalMisses = !isHit
        ? [...currentMisses, { row, col }]
        : currentMisses;
      ...
      const connections = await listConnectionsByRoom(roomCode);
      const nextTurnUserId = isVictory
        ? null
        : isHit
          ? cleanUserId
          : cleanOpponentId;
    ...
    if (action === "QUIT") {
      ...
      if (room.status === "IN_PROGRESS") {
        ...
        const cleanP1Id =
          p1 && p1.userId ? String(p1.userId).split(":")[0] : "";
