...
function Profile() {
  ...
  const nextChangeDate = attributes.lastUsernameChange
    ? new Date(
        new Date(attributes.lastUsernameChange).getTime() +
          30 * 24 * 60 * 60 * 1000,
      )
    : null;
  ...
  useEffect(() => {
    if (attributes.lastUsernameChange) {
      const daysSinceChange =
        (Date.now() - new Date(attributes.lastUsernameChange).getTime()) /
        (1000 * 60 * 60 * 24);
      if (daysSinceChange < 30) {
        setCooldownMessage(
          t("profile.cooldownMessageJs", {
            days: Math.ceil(30 - daysSinceChange),
          }),
        );
      } else {
        setCooldownMessage("");
  ...
  useEffect(() => {
    if (
      ...
      attributes.preferred_username === fullUsername
    ) {
      setIsChecking(false);
      setIsTaken(false);
      return;
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/users/check-username?username=${encodeURIComponent(fullUsername)}`,
        );
        if (response.ok) {
  ...
  const rankMeta = getRankMeta(
    hasRank ? attributes.rank || "bronze" : "bronze",
  );
  ...
  const rankLabel = hasRank
    ? `${getRankDisplayName(rankMeta)} - ${rankPoints} RP`
    : t("profile.unranked");
  const selectedIsUnranked = selectedRankId === "unranked";
  const selectedRank = selectedIsUnranked ? null : getRankMeta(selectedRankId);
  const selectedTitle = selectedIsUnranked
    ? t("profile.unranked")
    : getRankDisplayName(selectedRank);
  const currentRankIndex = hasRank
    ? RANKS.findIndex((rank) => rank.id === rankMeta.id)
    : -1;
  const selectedNextRank = selectedRank
    ? getNextRank(selectedRank.id)
    : RANKS[0];
  const selectedRankUnlocked =
    !!selectedRank && hasRank && rankPoints >= selectedRank.minRp;
  const selectedRankCurrent =
    !!selectedRank && hasRank && selectedRank.id === rankMeta.id;
  const selectedProgressStart = selectedRank?.minRp || 0;
  const selectedProgressEnd =
    selectedNextRank?.minRp || selectedRank?.minRp || RANKS[0].minRp;
  const selectedProgressRange = Math.max(
    1,
  ...
  const selectedProgressPercent = selectedIsUnranked
    ...
    : selectedRankCurrent
      ? Math.min(
          ...
          Math.max(
            0,
            ((rankPoints - selectedProgressStart) / selectedProgressRange) *
              100,
          ),
        )
  ...
  const rankRewards = {
    bronze: [
      t("profile.rewardBronze1"),
      t("profile.rewardBronze2"),
      t("profile.rewardBronze3"),
    ],
    silver: [
      t("profile.rewardSilver1"),
      t("profile.rewardSilver2"),
      t("profile.rewardSilver3"),
    ],
    gold: [
      t("profile.rewardGold1"),
      t("profile.rewardGold2"),
      t("profile.rewardGold3"),
    ],
    platinum: [
      t("profile.rewardPlatinum1"),
      t("profile.rewardPlatinum2"),
      t("profile.rewardPlatinum3"),
    ],
    diamond: [
      t("profile.rewardDiamond1"),
      t("profile.rewardDiamond2"),
      t("profile.rewardDiamond3"),
    ],
    master: [
      t("profile.rewardMaster1"),
      t("profile.rewardMaster2"),
      t("profile.rewardMaster3"),
    ],
    admiral: [
      t("profile.rewardAdmiral1"),
      t("profile.rewardAdmiral2"),
      t("profile.rewardAdmiral3"),
    ],
  };
  const unrankedRewards = [
    t("profile.rankRewardUnlockBronze"),
          ...
          <>
            ...
            <div className="profile-tabs-nav">
              ...
              >
                ...
                <span className="profile-tab-text-full">
                  {t("profile.serviceRecord")}
                </span>
                <span className="profile-tab-text-short">
                  {t("profile.serviceRecordShort")}
                </span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span className="profile-tab-text-full">
                  {t("profile.battleshipIdTitle")}
                </span>
                <span className="profile-tab-text-short">
                  {t("profile.battleshipIdTitleShort")}
                </span>
              </button>
              <button
                type="button"
                    ...
                    <div className="w-44 flex-shrink-0">
                      <HomeSelect
                        ...
                        options={[
                          {
                            value: "all",
                            label: language === "vi" ? "Tất cả" : "All Modes",
                          },
                          {
                            value: "ranked",
                            label: language === "vi" ? "Xếp hạng" : "Ranked",
                          },
                        ]}
                      />
                    </div>
                  </div>
                  <div className={`profile-service-rank-panel ${!hasRank ? 'is-unranked' : ''}`}>
                    <div className="profile-service-rank-header">
                      <div className="profile-service-rank-title-group">
                        <span className="profile-service-rank-kicker">
                          {t("profile.rank") || "Rank Tier"}
                        </span>
                        <span className="profile-service-rank-name">
                          {hasRank ? getRankDisplayName(rankMeta) : t("profile.unranked")}
                        </span>
                      </div>
                      {hasRank && (
                        <div className="profile-service-rank-badge-wrap">
                          <img 
                            src={rankMeta.badge} 
                            alt={getRankDisplayName(rankMeta)} 
                            className="profile-service-rank-badge"
                    ...
                    {hasRank && nextRank && (
                      <div className="profile-service-rank-progress-group">
                        <div className="profile-service-rank-progress-labels">
                          <span>{rankPoints} RP</span>
                          <span>{nextRank.minRp} RP</span>
                        </div>
                        <div className="profile-service-rank-progress-bar">
                          <div 
                            className="profile-service-rank-progress-fill" 
                            style={{ width: `${Math.min(100, Math.max(0, ((rankPoints - rankMeta.minRp) / (nextRank.minRp - rankMeta.minRp)) * 100))}%` }}
                          ></div>
                        </div>
                      </div>
                    )}
                    {hasRank && !nextRank && (
                      <div className="profile-service-rank-max-label">
                  ...
                  <dl>
                    ...
                    <div>
                      ...
                      <dd>
                        {rankLabel}
                        {hasRank && nextRank
                          ? ` - ${t("profile.rpToRank", { points: rpToNextRank, rank: getRankDisplayName(nextRank) })}`
                          : ""}
                      </dd>
                    </div>
                    <div>
              ...
              <section className="profile-riot-id-section">
                ...
                <div className="profile-riot-id-col-right">
                  <form
                    ...
                    className="profile-riot-id-form"
                  >
                    <div className="profile-riot-id-inputs-row">
                      <div className="profile-riot-id-input-group">
                        <label className="profile-riot-id-label">
                          {t("profile.commanderName")}
                        </label>
                        <input
                          type="text"
                          value={newUsername}
                      ...
                      <div
                        style={{
                          ...
                          color: isChecking
                            ...
                            : isTaken
                              ? "#ff4d4d"
                              : newUsername.length < 3 || newTag.length === 0
                                ? "#ff4d4d"
                                : "#4caf50",
                        }}
                      >
                        {newUsername.length > 0 &&
                          newUsername !==
                            (attributes.preferred_username?.split("#")[0] ||
                              "") &&
                          (isChecking
                            ? t("profile.checking")
                            : newUsername.length < 3
                              ...
                              : newTag.length === 0
                                ...
                                : isTaken
                                  ...
                                  : t("profile.nameValid"))}
                        {updateError && (
                          <div style={{ color: "#ff4d4d", marginTop: "4px" }}>
                            {updateError}
                          </div>
                        )}
                        {updateSuccess && (
                ...
                {loadingHistory ? (
                  ...
                  >
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="profile-match-card skeleton-card"
                        style={{ opacity: 0.9, pointerEvents: "none" }}
                      >
                        <div
                          className="profile-match-header"
                          style={{ opacity: 0.7 }}
                        >
                        ...
                        <div className="profile-match-players">
                          <div
                            className="profile-match-player is-p1"
                            style={{
                              display: "flex",
                              alignItems: "center",
                              gap: "12px",
                          ...
                          <div
                            className="profile-match-player is-p2"
                            style={{
                              display: "flex",
                              alignItems: "center",
                              gap: "12px",
                  ...
                  >
                    {matchHistory.map((match) => {
                      ...
                      const isP1You =
                        match.player1Id === match.userId ||
                        (currentUser?.userId &&
                          match.player1Id === currentUser.userId);
                      const isP2You =
                        match.player2Id === match.userId ||
                        (currentUser?.userId &&
                          match.player2Id === currentUser.userId);

                      const p1Acc =
                        match.player1Shots > 0
                          ? Math.round(
                              ((match.player1Shots -
                                (match.player1Misses || 0)) /
                                match.player1Shots) *
                                100,
                            )
                          : 0;
                      const p2Acc =
                        match.player2Shots > 0
                          ? Math.round(
                              ((match.player2Shots -
                                (match.player2Misses || 0)) /
                                match.player2Shots) *
                                100,
                            )
                          : 0;
                      ...
                      return (
                        <div
                          key={match.matchId}
                          className={`profile-match-card ${isWin ? "is-win" : "is-loss"}`}
                        >
                          {/* Title Bar */}
                          ...
                          <div className="profile-match-players">
                            ...
                            >
                              <img
                                src={
                                  match.player1Avatar
                                    ? `${getAvatarCdnUrl(match.player1Avatar)}?t=${Date.now()}`
                                    : COMMANDER_AVATAR
                                }
                                alt="Player 1"
                                ...
                                onError={(e) => {
                                  e.currentTarget.src = COMMANDER_AVATAR;
                                }}
                              />
                              <div
                                className="profile-match-player-info"
                                style={{
                                  display: "flex",
                                  flexDirection: "column",
                                }}
                              >
                                <span
                                  className="profile-match-player-name"
                                  style={{
                                    display: "inline-flex",
                                    alignItems: "center",
                                    fontWeight: "bold",
                                ...
                                >
                                  ...
                                  {match.leaverId === match.player1Id && (
                                    <span
                                      className="profile-surrender-text"
                                      style={{
                                        color: "#f44336",
                                        fontSize: "11px",
                                        marginLeft: "6px",
                                ...
                                <div className="profile-match-stats-row">
                                  ...
                                  <span className="profile-match-stat-pill accuracy">
                                    ...
                                    <span>
                                      {t("profile.accuracy")}:{" "}
                                      <strong className="val">{p1Acc}%</strong>
                                    </span>
                                  </span>
                                </div>
                              </div>
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  {match.leaverId === match.player2Id && (
                                    <span
                                      className="profile-surrender-text"
                                      style={{
                                        color: "#f44336",
                                ...
                                <div className="profile-match-stats-row is-right">
                                  <span className="profile-match-stat-pill accuracy">
                                    ...
                                    <span>
                                      {t("profile.accuracy")}:{" "}
                                      <strong className="val">{p2Acc}%</strong>
                                    </span>
                                  </span>
                                  <span className="profile-match-stat-pill misses">
                                    <span
                              ...
                              <img
                                src={
                                  match.player2Avatar
                                    ? `${getAvatarCdnUrl(match.player2Avatar)}?t=${Date.now()}`
                                    : COMMANDER_AVATAR
                                }
                                alt="Player 2"
                                className="profile-match-player-avatar"
